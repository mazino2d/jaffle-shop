...
SELECT
    ...
    DATEDIFF('day', dc.last_order_at, CURRENT_TIMESTAMP) AS days_since_last_order,
    ...
    -- Derived ratios
    dc.total_orders AS total_orders,
    CASE
    ...
    DATEDIFF('day', dc.first_order_at, CURRENT_TIMESTAMP) AS customer_tenure_days,

    -- Order cadence: avg days between orders (null if fewer than 2 orders)
    CASE
        WHEN dc.total_orders >= 2
        THEN DATEDIFF('day', dc.first_order_at, dc.last_order_at)
